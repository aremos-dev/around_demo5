...
def main():
    ...
    while True:
        # ser.write( bytes([0x01, 0x00, 0x01, 0x00, 0x00, 0xFF, 0xFF, 0xFF]))
        if ser.in_waiting:
            # 读取一个字节
            tool = ser.read(1)  
            # 如果没有数据，继续等待
            if not tool:
                continue
            # 喂给 TinyFrame
            tinyFrame.accept_byte(tool[0])  

             # 一帧收完了
            if tinyFrame.complete:
                # 返回相位
                if tinyFrame.rf.type == 0x0A13 and len(tinyFrame.rf.data) == 12:
                    floats = struct.unpack("<fff", tinyFrame.rf.data)
                    print(f"Received [total phase]: {floats[0]}")
                    print(f"Received [breath phase]: {floats[1]}")
                    print(f"Received [heart phase]: {floats[2]}")
                # 返回呼吸频率
                elif tinyFrame.rf.type == 0x0A14 and len(tinyFrame.rf.data) == 4:
                    floats = struct.unpack("<f", tinyFrame.rf.data)
                    print(f"Received [breath rate]: {floats[0]}")
                # 返回心跳频率
                elif tinyFrame.rf.type == 0x0A15 and len(tinyFrame.rf.data) == 4:
                    floats = struct.unpack("<f", tinyFrame.rf.data)
                    print(f"Received [heart rate]: {floats[0]}")
                # elif tinyFrame.rf.type == 0xFFFF and len(tinyFrame.rf.data) == 4:
                #     uint = struct.unpack("<BBBB", tinyFrame.rf.data)
                #     print(f"Received [version]: {uint}")
                # 存在检测
                elif tinyFrame.rf.type == 0x0F09 and len(tinyFrame.rf.data) == 2:
                    uint = struct.unpack("<BB", tinyFrame.rf.data)
                    print(f"Received [is there]: {uint[0]}")

                tinyFrame.reset_parser()    
