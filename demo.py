...
class dot():
    ...
    def mode1(self):
        print('mode1: gentle tap response')
        if self.is_levitating:
            self.guide_finished()
            return
        self.ble.color_sync(255, 200, 0)  # warm yellow flash
    ...
    def mode2(self):
        print('mode2: slow roll / idle play response')
        duration = 8.0
        start = time.time()
        self.ble.color_sync(80, 0, 255)  # blue-purple slow flow
    ...
    def wait_to_accumulate(self,):
        ...
        self.le = False
        self.is_levitating = False
        self.music('breath.WAV', max_duration=10)
        self.le = True
        self.ble.mode_sync(3)
    ...
    def _engaged_loop(self):
        ...
        while self.state == 'engaged':
            if not self.is_levitating:
                print('start interaction monitoring')
                time.sleep(10)
                ...
                while not self.is_levitating:
                    ...
                    if self.ble.gyroscope[-1] == 1:
                        self.need_mode1()
                        return
                    elif self.ble.gyroscope[-1] == 2:
                        self.need_mode2()
                        return
                    elif self.ble.gyroscope[-1] == 3:
                        self.need_mode3()
                        return
    ...
    def mode3(self):
        ...
        duration = 12.0
        start = time.time()
        self.ble.color_sync(255, 165, 0)  # orange fast flow
        self.ble.freq_light_sync(12)
        self.ble.spike_shake_sync(1)
    ...
    def _enter_guiding_mode1(self, event=None):
        print('usr take the dot (mode1 / mindset wander)')
